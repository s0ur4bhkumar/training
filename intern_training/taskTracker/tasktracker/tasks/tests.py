...
class TasksListFilterTests(TestCase):
    def setUp(self):
        Task.objects.create(
            title="Buy groceries", description="Milk and bread", due_date="2026-09-12"
        )
        Task.objects.create(
            title="Plan trip", description="Book hotel", due_date="2026-09-15"
        )
        Task.objects.create(
            title="Write report", description="Draft summary", due_date="2026-09-12"
        )
